...
import { CreatureModel, getAttributes, emptyAttributes, emptyCreature, getCreatingCreature, ProgramInfo, AttributeType, resourceTypes, ResourceAmountPair } from './models';
 ...
 */
function rawToModel(raw: CreatureRaw, index: number, globalTimer: number, rawcards: any[]): CreatureModel {
    ...
    extraReducers: (builder) => {
      builder
        .addCase(queryState.fulfilled, (state, action) => {
            ...
            state.creatures = creatures.map((creature, index) => rawToModel(creature, index, globalTimer, rawcards));
        });
    }
  },
);
 ...
 */
export function adjustProcessingTimeBySpeed(originalTime: number, speedValue: number): number {
...
export const selectSelectedCreatureDiffResources = (state: RootState) => {
    ...
    const programs = selectProgramsByIndexes(selectedCreature.programIndexes)(state).filter(program => program != null);
    ...
    const productivityValue = selectedCreature.attributes.find((attr: { type: AttributeType, amount: number }) => attr.type === AttributeType.Productivity)?.amount ?? 0;
    const EfficiencyValue = selectedCreature.attributes.find((attr: { type: AttributeType, amount: number }) => attr.type === AttributeType.Efficiency)?.amount ?? 0;
...
const getCurrentProgram = (selectedCreature: CreatureModel) => (localTimer: number) => (state: RootState): ProgramInfo => {
    ...
    const adjustedProcessingTime = adjustProcessingTimeBySpeed(program.processingTime, speedValue);

    const time = localTimer - selectedCreature.startTime;
    return {
        program,
        index: currentProgramIndex,
        remainTime: Math.min(Math.ceil(adjustedProcessingTime - time), adjustedProcessingTime),
